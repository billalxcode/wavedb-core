...
import { getMetricQueryValidation, writeMetricValidation } from "@/validations/metric.validation";

export async function writeMetricController(c: Context<{ Variables: AuthVariables }>) {
...
export async function getMetricsController(c: Context<{ Variables: AuthVariables }>) {
	...
	if (!queriesValidated.success) {
		throw new ValidationError(queriesValidated.error.issues, "validation failed", 400)
	}
	...
	return c.json({
		...
		data: await metricService.getMetricsByRunnerId(runnerId, queriesValidated.data)
	})
}
