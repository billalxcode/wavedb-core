...
export const getMetricQueryValidationSchema = z.object({
	orderBy: z.enum(["step_asc", "step_desc", "time_asc", "time_desc"]).optional(),
	limit: z.number().min(1, "limit must be at least 1").max(5_000, "limit cannot exceed 5000").optional(),
	offset: z.number().min(0, "offset must be a non-negative integer").optional()
})

export type WriteMetricValidationType = z.infer<typeof writeMetricValidationSchema>;
export type GetMetricQueryValidationType = z.infer<typeof getMetricQueryValidationSchema>;
